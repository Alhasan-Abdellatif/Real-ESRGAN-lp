...
class PatchDiscriminator(nn.Module):
    ...
    """
    def __init__(self, num_in_ch=3,num_feat = 64,n_layers_D=4,SN= False,norm_layer = None):
        super(PatchDiscriminator, self).__init__()   
        nf = num_feat
        conv_fun =  self.conv4x4   
        ...
        sequence = [conv_fun(num_in_ch, num_feat,SN=SN,bias = True),
    ...
    def conv4x4(self,ch_in,ch_out,SN = False,s = 2,p=1,bias = True):
        if SN:
            return spectral_norm(nn.Conv2d(ch_in, ch_out, kernel_size=4, padding=p,stride=s,bias = bias))
        else:
